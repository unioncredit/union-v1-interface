...
export async function stake(tokenAddress, amount, signer, chainId) {
  ...
  try {
    ...
    if (allowanceAmount.lt(stakeAmount)) {
      await erc20Token.approve(STAKING_MANAGER_ADDRESSES[chainId], MaxUint256);
    }
    ...
    let estimate;
    try {
      estimate = await contract.estimateGas.stake(
        tokenAddress,
    ...
    return contract.stake(tokenAddress, stakeAmount.toString(), {
      gasLimit: estimate,
    });
  } catch (err) {
    throw err;
