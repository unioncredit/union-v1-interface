...
import { MEMBER_MANAGER_ADDRESSES, UNION_TOKEN_ADDRESSES } from "@constants/";
...
export async function verifyMembership(account, tokenAddress, signer, chainId) {
  ...
  try {
    ...
    const unionContract = await getContract(
    ...
    const allowanceRes = await unionContract.allowance(userAddress, MEMBER_MANAGER_ADDRESSES[chainId]);
    const allowanceAmount = new BigNumber(allowanceRes.toString());
    if (allowanceAmount.lt(fee)) {
      const tx = await unionContract.approve(MEMBER_MANAGER_ADDRESSES[chainId], fee.toFixed());
      await tx.wait();
    }
