...
export async function verifyMembership(account, tokenAddress, signer, chainId) {
  ...
  try {
    ...
    const allowanceRes = await unionContract.allowance(userAddress, UNION_TOKEN_ADDRESSES[chainId]);
    const allowanceAmount = new BigNumber(allowanceRes.toString());
    if (allowanceAmount.lt(fee)) {
      const tx = await unionContract.approve(UNION_TOKEN_ADDRESSES[chainId], fee.toFixed());
      await tx.wait();
    }
