...
export async function getRewardsPerYear(tokenAddress, signer, chainId) {
  ...
  try {
    ...
    const userBlockDelta = await contract.getUserBlockDelta(userAddress, tokenAddress);
    let blocks;
     ...
     */
    const res = await contract.calculateRewardsByBlocks(userAddress, tokenAddress, blocks);
