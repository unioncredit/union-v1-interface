...
export async function getVouched(account, tokenAddress, signer, chainId) {
    ...
    try {
         ...
         */
        const addresses = await contract.getStakerAddresses(
            account,
            tokenAddress
        );
        ...
        const list = await Promise.all(
            addresses.map(async (address) => {
                ...
                const totalUsed = Number(
                    formatUnits(
                        await contract.getTotalLockedStake(
                            address,
                            tokenAddress
                        ),
                        18
                ...
                const stakingAmount = Number(
                    formatUnits(
                        await stakingContract.getStakerBalance(
                            address,
                            tokenAddress
                        ),
                        18
                ...
                const vouched = Number(
                    formatUnits(res.vouchingAmount, 18)
                ).toFixed(2);

                const used = Number(formatUnits(res.lockedStake, 18)).toFixed(
                    2
                );
                ...
                const trust = Number(formatUnits(res.trustAmount, 18)).toFixed(
                    2
                );
