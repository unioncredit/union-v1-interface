...
import { getVouchTotal } from "util/getVouchBarData";
  ...
  try {
    ...
    const list = await Promise.all(
      addresses.map(async (address) => {
        ...
        const isActive = await contract.isActiveVouch(
          address,
          account,
          tokenAddress
        );
        ...
        const health = isOverdue ? (Number(available) / vouched) * 100 : 0;

        const pct = Number(parseFloat(available) / parseFloat(creditLimit));

        const percentage = pct > 1 ? 1 : pct;
