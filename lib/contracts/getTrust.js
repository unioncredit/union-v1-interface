  ...
  try {
    ...
    const list = await Promise.all(
      addresses.map(async (address) => {
        ...
        const isActive = await contract.checkAssetIsActive(account, address, tokenAddress);

        const vouched = Number(formatUnits(res.vouchingAmount, 18));

        const used = Number(formatUnits(res.creditUsed, 18));

        const trust = Number(formatUnits(res.trustAmount, 18));

        const percentage = parseFloat(res.creditUsed / res.vouchingAmount);
