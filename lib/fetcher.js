...
import { getTrustCount } from "./contracts/getTrustCount";
...
const getAllTransactions = async (curToken, library, chainId) => {
  ...
  return [...borrow, ...repay].sort((a, b) => a.blockNumber - b.blockNumber);
};

/**
 * @name fetcher
 * @param {("trust"|"vouch"|"transactions"|"trustcount")} key
 */
export default (key, { account, curToken, library, chainId }) => {
  ...
  if (key === "trustcount")
    return getTrustCount(account, curToken, library, chainId);
