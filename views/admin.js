...
export default function BorrowView() {
        ...
        async function fetchUnionData() {
            try {
                if (isMounted) {
                    ...
                    setMinInflation((parseRes(minInflationPerBlockRes, 18) * BLOCKS_PER_YEAR[chainId] * 100).toFixed(2));
                    setMaxInflation((parseRes(maxInflationPerBlockRes, 18) * BLOCKS_PER_YEAR[chainId] * 100).toFixed(2));
    ...
    const onSetOriginationFee = useAutoCallback(async (amount, res) => {
        try {
            await marketContract.setOriginationFee(new BigNumber(amount / 100).times(WAD).toFixed());
        } catch (err) {
            console.error(err);
        }
    });
    ...
    const onSetDebtCeiling = useAutoCallback(async (amount) => {
        try {
            await marketContract.setDebtCeiling(new BigNumber(amount).times(WAD).toFixed());
        } catch (err) {
            console.error(err);
        }
    });
    ...
    const onSetMinLoan = useAutoCallback(async (amount) => {
        try {
            await marketContract.setMinLoan(new BigNumber(amount).times(WAD).toFixed());
        } catch (err) {
            console.error(err);
        }
    });
    ...
    const onSetNewMemberFee = useAutoCallback(async (amount) => {
        try {
            await memberContract.setNewMemberFee(new BigNumber(amount).times(WAD).toFixed());
        } catch (err) {
            console.error(err);
        }
    });
    ...
    const onSetDefaultInflation = useAutoCallback(async (amount) => {
        try {
            await unionContract.setDefaultInflationPerBlock(new BigNumber(amount / 100).times(WAD).toFixed());
        } catch (err) {
            console.error(err);
        }
    });
    ...
    const onSetMaxInflation = useAutoCallback(async (amount) => {
        try {
            await unionContract.setMaxInflationPerBlock(new BigNumber(amount / (100 * BLOCKS_PER_YEAR[chainId])).times(WAD).toFixed(0));
        } catch (err) {
            console.error(err);
        }
    });
    ...
    const onSetMinInflation = useAutoCallback(async (amount) => {
        try {
            await unionContract.setMinInflationPerBlock(new BigNumber(amount / (100 * BLOCKS_PER_YEAR[chainId])).times(WAD).toFixed(0));
        } catch (err) {
            console.error(err);
        }
    });
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td>{minLoan}</td>
                                <td>{overdueBlocks}</td>
                                <td><Button disabled={!isAdmin} onClick={toggleMarketModal}>Edit</Button></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                    ...
                    <table>
                        ...
                        <tbody>
                            <tr>
                                ...
                                <td><Button disabled={!isAdmin} onClick={toggleManagerModal}>Edit</Button></td>
                            </tr>
                        </tbody>
                    </table>
                </div>
