...
export default function AdminView() {
    ...
    useAutoEffect(() => {
        ...
        async function fetchUnionData() {
            try {
                if (isMounted) {
                    ...
                    setDefaultInflation(
                        parseRes(defaultInflationPerBlockRes, 2) * 100
                    );
                }
            } catch (err) {
                if (isMounted) {
        ...
        async function fetchStakingData() {
            try {
                if (isMounted) {
                    const totalStakedRes = await stakingContract.totalStaked(
                        curToken
                    );
                    const totalFrozenRes = await stakingContract.totalFrozen(
                        curToken
                    );
